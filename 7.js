...
class EmployeeGroup {
  ...
  removeEmployee(employee) {
    const index = this.employees.findIndex(empl => empl === employee);
    if (index !== -1) {
...
class EmployeeIterator {
  ...
  constructor(employeeGroup) {
    this.employees = employeeGroup.getEmployees();
    this.currentIndex = 0;
  }
  ...
  next() {
    return this.hasNext() ? this.employees[this.currentIndex++] : null;
  }
